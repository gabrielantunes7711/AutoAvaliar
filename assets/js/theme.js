...
const swiper = new Swiper(".slide-testimonies", {
  navigation: {
    nextEl: ".btn-next",
    prevEl: ".btn-prev",
  },
...
/* ------- Password Visibility ------- */
const allPasswordVisibilityBtn = document.querySelectorAll(
  ".container-password button"
);

for (const btn of allPasswordVisibilityBtn) {
...
/* ------- Lightbox ------- */
const allVideoSrc = document.querySelectorAll("[data-iframe-src]");

for (const videoSrc of allVideoSrc) {
  videoSrc.addEventListener("click", toggleLightbox);
}

function toggleLightbox(e) {
  const el = e.currentTarget;
  const srcVideo = el.dataset.iframeSrc;
  ...
  if (lightboxModal) {
    ...
    createLightboxModal();
    populateLightboxModal(srcVideo);
  }
}

function getYouTubeSrc(src) {
...
function createLightboxModal() {
  ...
  button.innerHTML = closeX;
  lightboxModal.prepend(button);
  document.body.prepend(lightboxModal);

  lightboxModal.addEventListener("click", toggleLightbox);
}

function populateLightboxModal(src) {
  const videoModal = document.querySelector(".lightbox-modal");

  videoModal.innerHTML += `<iframe id="iframeHome" width="80%" height="80%" src="https://www.youtube.com/embed/${getYouTubeSrc(
    src
